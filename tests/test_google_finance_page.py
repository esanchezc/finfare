...
def test_should_get_six_stocks_you_may_be_interested_in(displayed_symbols):
    assert (
        len(displayed_symbols) == 6
    ), f"Expected 6 stock symbols, {len(displayed_symbols)} found"
...
def test_compare_stock_symbols(displayed_symbols):
    ...
    print_stock_list(
        "\nStocks you might be interested that are not in the test data:",
        not_in_test_data,
    )

    assert (
        found_symbols
    ), "None of the expected stock symbols were found in the watchlist"
    print(
...
def test_print_test_data_stocks_not_in_interested_stocks(displayed_symbols):
    ...
    print_stock_list(
        "\nStocks in test data that are not in stocks you might be interested in:",
        missing_stocks,
    )
